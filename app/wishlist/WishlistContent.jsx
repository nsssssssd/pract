...
function WishlistItem({ product, index, removeItem, addToCart, openCart }) {
  const x = useMotionValue(0);
  const [isRemoving, setIsRemoving] = useState(false);

  // Background trash reveal animation tied to swipe progress
  const progress = useTransform(x, [-SWIPE_MAX, 0], [1, 0]);
  const trashScale = useTransform(progress, [0, 1], [0.6, 1]);
  const trashOpacity = useTransform(progress, [0, 0.4, 1], [0, 1, 1]);
  ...
  async function handleDragEnd(_, info) {
    ...
    if (offset < -SWIPE_THRESHOLD || velocity < -500) {
      setIsRemoving(true);
      await animate(x, -SWIPE_MAX, { type: 'tween', duration: 0.2 });
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: isRemoving ? 0 : 1, y: 0, height: isRemoving ? 0 : 'auto' }}
    ...
    >
      ...
      <div className="absolute inset-y-0 right-0 w-full flex items-center justify-end bg-destructive rounded-2xl px-5">
        <motion.div style={{ scale: trashScale, opacity: trashOpacity }}>
